...
router.put("/notices/:id", uploadNotice.single("image"), async (req, res) => {
  try {
    const { title } = req.body;
    if (!title) return res.status(400).json({ error: "Title is required" });
    if (req.file) {
      const relativePath = `/uploads/notices/${req.file.filename}`;
      await pool.query("UPDATE notices SET title = ?, image_path = ? WHERE id = ?", [
        title,
        relativePath,
        req.params.id,
      ]);
    } else {
      await pool.query("UPDATE notices SET title = ? WHERE id = ?", [title, req.params.id]);
    }
    await logAction(req.user.id, "notice.edited", { noticeId: Number(req.params.id), title });
